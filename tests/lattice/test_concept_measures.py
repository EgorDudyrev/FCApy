...
def test_stability():
    ...
    stabs_est = [c.measures.get('Stab') for c in ltc.concepts]
...
def test_stability_bounds():
    ...
    lstabs_est = [c.measures.get('LStab') for c in ltc.concepts]
    ...
    mae = sum([abs(lstabs_true[c_i]-lstabs_est[c_i])
               if len(c.extent_i) > 0 else 0 for c_i, c in enumerate(ltc.concepts)]
              )/len(lstabs_true)
