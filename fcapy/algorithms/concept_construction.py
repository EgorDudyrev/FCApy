...
def sofia_objectwise(
    ...
    for proj_i in proj_iterator:
        ...
        if len(L_proj) > L_max:
            ...
            thold = sorted(measure_values)[-L_max - 1]
...
def sofia_binary(context: FormalContext, L_max=100, iterate_attributes=True, measure='LStab',
    ...
    if isinstance(measure, tuple) and len(measure) == 2 and type(measure[0]) == str and type(measure[1]) != str:
        ...
        raise TypeError(
            ...
            "It should be either a measure: str"
            "or a pair of (measure: str, measure_func: c_i, lattice, context -> float)")
    ...
    for projection_num in utils.safe_tqdm(range(proj_to_start+1, max_projection + 1),
        ...
        new_concepts = close_by_one(
            ...
            iter_concepts_to_check=[projection_num-1]
        )
        ...
        if len(lattice) > L_max:
            for m in measure:
                lattice.calc_concepts_measures(m, ctx_projected)


            metrics = [c.measures[measure_name] for c_i, c in enumerate(lattice)]
...
def sofia_general(context: MVContext, L_max=100, measure='LStab', proj_to_start=None, use_tqdm=False):
    ...
    of the original Sofia algorithm. Thus the function may work not as efficient as expected.
    Moreover it represents the simplest way construct a lattice over MVContext. Therefore it is very inefficient.
