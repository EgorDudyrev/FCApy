...
if LIB_INSTALLED['bitarray']:
    ...
    from bitarray.util import zeros as zerobar
...
class BinTableBitarrays(AbstractBinTable):
    data: List[fbitarray]  # Updating type hint
    Row_DType = fbitarray

    @property
    def T(self) -> 'BinTableBitarrays':
    ...
    def __eq__(self, other: 'BinTableBitarrays'):
        if self.height != other.height:
            return False
        if self.width != other.width:
            return False
    ...
    def __and__(self, other: 'BinTableBitarrays') -> 'BinTableBitarrays':
        assert self.shape == other.shape
        intersection = [row_a & row_b for row_a, row_b in zip(self.data, other.data)]
        return self.__class__(intersection)

    def __or__(self, other: 'BinTableBitarrays') -> 'BinTableBitarrays':
        assert self.shape == other.shape
        intersection = [row_a | row_b for row_a, row_b in zip(self.data, other.data)]
        return self.__class__(intersection)

    def __invert__(self) -> 'BinTableBitarrays':
        data_neg = [~row for row in self.data]
        return self.__class__(data_neg)


class BinTableBitarray(AbstractBinTable):
    data: Tuple[fbitarray, int]  # Updating type hint  (bitarray, number of rows)
    Row_DType = fbitarray
    ...
    @staticmethod
    def _transform_data_fromlists(data: List[List[bool]]) -> Tuple[fbitarray, int]:
        h, w = len(data), len(data[0])
        bar = zerobar(h*w)

        offset = 0
        for i in range(h):
            bar[offset:offset + w] |= fbitarray(data[i])
            offset += w

        return fbitarray(bar), h

    def _validate_data(self, data: Tuple[fbitarray, int]) -> bool:
        bar, h = data
        if len(bar) == 0:
            return True
    ...
    def _all_per_row(self, rows: List[int] = None, columns: List[int] = None) -> Row_DType:
        ...
        return fbitarray([self._get_row(i, columns).all() for i in rows])

    def _all_per_column(self, rows: List[int] = None, columns: List[int] = None) -> Row_DType:
        columns = range(self.width) if columns is None else columns

        return fbitarray([self._get_column(rows, j).all() for j in columns])
    ...
    def _any_per_row(self, rows: List[int] = None, columns: List[int] = None) -> Row_DType:
        ...
        return fbitarray([self._get_row(i, columns).any() for i in rows])

    def _any_per_column(self, rows: List[int] = None, columns: List[int] = None) -> Row_DType:
        columns = range(self.width) if columns is None else columns

        return fbitarray([self._get_column(rows, j).any() for j in columns])
    ...
    def _get_column(self, row_slicer: List[int] or slice or None, column_idx: int) -> Row_DType:
        ...
        return fbitarray([column[row_i] for row_i in row_slicer])
...
BINTABLE_CLASSES = {cl.__name__: cl for cl in [BinTableLists, BinTableNumpy, BinTableBitarray, BinTableBitarrays]}
BINTABLE_DEPENDENCY_DICT = {'BinTableBitarray': {'bitarray'}, 'BinTableBitarrays': {'bitarray'},
                            'BinTableNumpy': {'numpy'}, 'BinTableLists': set()}
